import { Camera, CheckCircle2, Download, Eye, Images, Send, Trash2, X, ZoomIn, ZoomOut } from "lucide-react";
...
const CANVAS_SENT_TOAST_DURATION_MS = 2400;
...
export function DirectorDeskCaptureToast({ onClose }: { onClose: () => void }) {
  return (
    <div className="director-canvas-toast" role="status" aria-live="polite">
      <CheckCircle2 aria-hidden="true" size={18} strokeWidth={2.2} />
      <span>截图已添加到画布</span>
      <button
        className="director-canvas-toast-close"
        type="button"
        aria-label="关闭提示"
        onClick={onClose}
      >
...
export function CameraPanel() {
  ...
  const [canvasSentToastVisible, setCanvasSentToastVisible] = useState(false);
  ...
  const canvasSentToastTimerRef = useRef<number | null>(null);
  ...
  useEffect(() => {
    return () => {
      if (canvasSentToastTimerRef.current !== null) {
        window.clearTimeout(canvasSentToastTimerRef.current);
      }
    };
  }, []);
  ...
  const hideCanvasSentToast = useCallback(() => {
    if (canvasSentToastTimerRef.current !== null) {
      window.clearTimeout(canvasSentToastTimerRef.current);
      canvasSentToastTimerRef.current = null;
    }

    setCanvasSentToastVisible(false);
  }, []);

  const showCanvasSentToast = useCallback(() => {
    if (canvasSentToastTimerRef.current !== null) {
      window.clearTimeout(canvasSentToastTimerRef.current);
    }

    setCanvasSentToastVisible(true);
    canvasSentToastTimerRef.current = window.setTimeout(() => {
      setCanvasSentToastVisible(false);
      canvasSentToastTimerRef.current = null;
    }, CANVAS_SENT_TOAST_DURATION_MS);
  }, []);
  ...
  const sendAllCapturesToCanvas = useCallback(() => {
    const payload = cameraCaptureGroups.flatMap((group) =>
    ...
    postDirectorDeskCapturesToHost(payload);
    if (payload.length > 0) {
      showCanvasSentToast();
    }
  }, [cameraCaptureGroups, showCanvasSentToast]);
  ...
  return (
    <>
      {canvasSentToastVisible ? <DirectorDeskCaptureToast onClose={hideCanvasSentToast} /> : null}
      <InspectorPanel
        title="摄像机"
        ariaLabel="摄像机右侧属性面板"
