...
export interface DirectorDeskCaptureToCanvas {
  dataUrl: string;
  fileName: string;
}
...
let internalCaptureHandler: DirectorDeskCaptureHandler | null = null;
...
export function postDirectorDeskCapturesToHost(
  ...
  if (internalCaptureHandler) {
    void internalCaptureHandler(normalizedCaptures);
    return;
  }
...
export function setDirectorDeskCaptureHandler(handler: DirectorDeskCaptureHandler) {
  internalCaptureHandler = handler;
}

export function clearDirectorDeskCaptureHandler(handler?: DirectorDeskCaptureHandler) {
  if (!handler || internalCaptureHandler === handler) {
    internalCaptureHandler = null;
  }
}
